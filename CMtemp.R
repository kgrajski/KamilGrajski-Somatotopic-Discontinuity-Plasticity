...
CMPointByPoint = function ( ctl.CMRaw.e, exp.CMRaw.e, ctl.CMRaw.i, exp.CMRaw.i,
	...
	for ( i in 1:N2 ) {
		...
		if ( length(tmp1)>0 & length(tmp2)>0 ) {
			...
			ctl.MeanRFArea.e[i] = mean(ctl.RFArea.e[TrimEdgesFromCellList ( tmp1.List, N, iTrimRings )]);
			exp.MeanRFArea.e[i] = mean(exp.RFArea.e[TrimEdgesFromCellList ( tmp2.List, N, iTrimRings )]);
		...
		if ( length(tmp1)>0 & length(tmp2)>0 ) {
			...
			ctl.MeanRFArea.i[i] = mean(ctl.RFArea.i[TrimEdgesFromCellList ( tmp1.List, N, iTrimRings )]);
			exp.MeanRFArea.i[i] = mean(exp.RFArea.i[TrimEdgesFromCellList ( tmp2.List, N, iTrimRings )]);

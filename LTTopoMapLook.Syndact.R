...
#iExpName = "Syndact.";
iExpName = "Syndact.Release.";

iSyndBase = 10;
iRefBase = 25;

if ( iExpName == "Syndact." ) {
	iStart = 1; iEnd = iSyndBase;
} else {
	iStart = 1; iEnd = iRefBase;
} # if ( iExpName == "Syndact."
...
for ( iRefinement in iStart:iEnd ) {
	...
	if ( iExpName == "Syndact." ) {
		if ( iCount == 1 ) {
			fileName = paste ( "Run.", iRefBase, ".", iRefBase, sep="" );
		} else {
			fileName = paste ( "Syndact.", iSyndZone, ".", iSyndBase, ".", iRefinement, sep="" );
		} # if ( iCount == 1 )
	} else {
		if ( iCount == 1 ) {
			fileName = paste ( "Syndact.", iSyndZone, ".", iSyndBase, ".", iSyndBase, sep="" );
		} else {
			fileName = paste ( "Syndact.Release.", iZone, ".", iBase, ".", iRefinement, sep="" );
		} # if ( iCount == 1 )
	} # if ( iExpName == "Syndact." )
...
if ( iExpName == "Syndact." ) {
	save ( file=paste("TopoMapData.Syndact.", iZone, ".RData", sep=""), list=c("evol.rfTrackData.e", "evol.rfTrackData.i", "evol.cortical.Amp.e", "evol.cortical.Amp.i" ) );
	load ( file=paste("TopoMapData.Syndact.", iZone, ".RData", sep="") );
} else {
	save ( file=paste("TopoMapData.Syndact.Release.", iZone, ".RData", sep=""), list=c("evol.rfTrackData.e", "evol.rfTrackData.i", "evol.cortical.Amp.e", "evol.cortical.Amp.i" ) );
	load ( file=paste("TopoMapData.Syndact.Release.", iZone, ".RData", sep="") );
} # if ( iExpName == "Syndact." )
...
ylim = c ( 0, max(cAmp.i) );
plot( cAmp.i[,1], type="l", lty=1, col=1, ylim=ylim, main="Evolution Input Layer Amplification in I-Cells",
xlab="Iterations", ylab="Area",sub="Digits: 3(Solid), 2(Dash), 1(Dot); Sector: Distal(Blk), Mid(Red), Prox(Green)" );
